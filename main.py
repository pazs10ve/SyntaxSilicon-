...
def display_file_content_window(title, filename):
    """Display file content in a separate Tkinter window"""
    try:
    ...
    text_area = scrolledtext.ScrolledText(window, wrap=tk.WORD, font=("Courier New", 10))
...
def main():
    try:
        ...
        # Create root Tkinter window (hidden)
        root = tk.Tk()
        root.withdraw()
        ...
        while True:
            ...
            try:
                ...
                print('Saved the Verilog code to generated_module.v')
                
                # Display saved Verilog file content
                display_file_content_window("Generated Verilog Code (generated_module.v)", "generated_module.v")
                
                # Generate and save testbench using PyVerilog
                testbench_code = generate_testbench()
                print("\nGenerated Testbench Code:\n")
                print(testbench_code)
                testbench_file = save_testbench(testbench_code)
                print('Saved the testbench code to testbench.v')
                
                # Display saved Testbench file content
                display_file_content_window("Generated Testbench Code (testbench.v)", "testbench.v")
                
                # Simulate
                if simulate_verilog_code(verilog_file, testbench_file):
